...
Pane paste/drop operations use the shared background FileShelfTransfer path. Recycle, permanent deletion and undo/redo execute on a short-lived STA worker; completion and history bookkeeping return to the UI thread. Pane mutations, shelf transfers and undo/redo consult a shared busy lifetime, held through history recording and observed by shutdown/update. This serializes these in-app operations; it does not coordinate external tools or provide a durable queue. Rename history snapshots run in the background; simple create actions still include synchronous work. CopyFileEx provides byte progress and cancellation during staged file copies while retaining Windows stream/metadata handling.
