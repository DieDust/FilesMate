...
public enum SearchFilter { All, Apps, Documents, Images, Media, Folders, Executables }
...
public static class SearchFilters
{
    public static bool Matches(SearchFilter filter, string path, bool directory)
    {
        ...
        {
            SearchFilter.Apps => kind == FilesMate.App.Models.SearchHitKind.Program,
            SearchFilter.Executables => kind == FilesMate.App.Models.SearchHitKind.Executable,
...
public sealed class IndexedFilesSearchProvider(string? profileDirectory = null, bool launcherFiles = false, IReadOnlyList<string>? extensions = null) : IGlobalSearchProvider
{
    public async Task<GlobalSearchResponse> SearchAsync(string query, CancellationToken cancellationToken, SearchFilter filter = SearchFilter.All, int limit = 40, int offset = 0)
    {
        ...
        var includeExecutables = SearchExecutableConfiguration.Load(profileDirectory);
        bool Include(string path, bool directory) => !hidden.Contains(HiddenSearchResults.FileKey(path)) &&
            (includeExecutables || !ApplicationCatalog.IsIndexedExecutable(path, directory)) &&
            (extensions is not null ? !directory && extensions.Contains(Path.GetExtension(path), StringComparer.OrdinalIgnoreCase)
                : SearchFilters.Matches(filter, path, directory));
