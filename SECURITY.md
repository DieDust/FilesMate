...
- PDF, Markdown and generated Office previews disable page scripts, web messages, permission grants, downloads and new browser windows. Markdown is rendered with raw HTML disabled and a restrictive content security policy. Explicit web links may open in the default browser; local Markdown image references use a virtual mapping of the document directory.
- Office conversion is performed in a separate process with input/output limits and a timeout. This is crash/timeout isolation, **not an OS security sandbox or a memory quota**. Third-party parsers and the installed WebView2 Runtime remain part of the attack surface.
